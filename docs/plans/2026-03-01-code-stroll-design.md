# /code-stroll: Interactive Code Review Learning Mode
**Date:** 2026-03-01
**Status:** Approved
...
The core insight: the most important skill in the age of agentic coding is knowing *what you need to deeply understand* vs. what you can safely delegate. `/code-stroll` serves the "deeply understand" mode.
...
```
.opencode/commands/code-stroll.md     ← entry point (slash command)
...
```
User: /code-stroll --depth deep --focus auth

Plugin:
  1. Parse flags from BEGIN_CODE_STROLL sentinel
...
|---|---|---|---|
| `BEGIN_CODE_STROLL depth=... focus=... resume=... base=...` | Command file | Plugin | Kicks off pre-analysis pass |
...
export default function codeStrollPlugin(opencode: OpenCodePlugin) {
  opencode.on("session.message", async (event) => {
    ...
    // Entry point
    if (content.match(/^BEGIN_CODE_STROLL/)) {
...
**Path:** `.opencode/commands/code-stroll.md`
...
**Usage:**
```
/code-stroll                          # review current branch vs main, depth=deep
/code-stroll --depth skim             # flag concerns only
/code-stroll --focus auth,api         # only review changes in these directories
...
| opencode crash mid-session | Worktree left on disk; reused on next `/code-stroll` for same branch |
...
| Plugin integration | `BEGIN_CODE_STROLL` → pre-analysis → chunk injection | Integration test with fixture git repo |
